...
系统分别统计 Score ≥ 6/7/8/9/10 后 3、5、10、20、60 个交易日的胜率、平均收益、中位收益、最大回撤、盈亏比和样本量，并单列普通交易日、月末/月初、季度末/季度初。季度窗口是否有效由数据结果判断，不预设结论。

防未来函数措施：所有指标在信号日逐行计算；新低和均量基准先排除当日；每个回测日期传给评分函数的行情在该日截断；基本面执行 `as_of` 过滤；未来收益仅在评分冻结后由独立函数计算。
