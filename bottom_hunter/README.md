...
- **总览**：显示合并后的自选统计，扫描最新/指定日期，运行回测并查看实时日志。
- **我的自选**：查看加密货币、美港股、A 股、跨来源重合和链上股票标识，按类别/行业过滤，也可人工修正股票行业。
- **研究中心**：按自选股查看财务指标、财报/公告、新闻、媒体与雪球等社区观点；单独查看增长、通胀、流动性和风险偏好。
- **报告中心**：按时间浏览 Markdown 日报和回测报告，也可以交给系统默认应用打开。
- **自选导入**：同花顺、币安和欧易统一通过文件或手动添加维护，不需要平台 API Key；长桥认证行情作为独立的可选数据源保留。
- **系统状态**：检查 Qt 桌面组件、配置、SQLite、最新日报，并查看最近扫描批次及 `success/partial/aborted` 状态。
...
- 宏观数据默认使用 FRED CSV，指标和行业映射可在 [config/research.yaml](config/research.yaml) 修改。界面会显示观测日期、前值、变化和 `-2～+2` 方向信号。
...
个股名义满分 11 分：超跌 2、恐慌抛售 2、拒绝创新低 2、支撑位 1、板块宽度 1、基本面 2、时间窗口 1。支撑位取目标日之前历史摆动低点（可选叠加恐慌低点锚），当日回踩支撑区间并收于其上记 1 分。压力位为同期历史摆动高点的配套判断（不计分）：收盘突破临近压力位视为压力转支撑参考；收盘紧贴压力位下方（3% 内）提示反弹空间受限并写入风险。基本面为 `N/A` 时显示为 `x/9`，信号最高限制在 `EARLY REVERSAL`；用户补入可靠数据后才恢复 11 分评价。
...
可选推送：复制 `config/notify.example.yaml` 为 `notify.yaml`，填入凭据并设 `enabled: true`。到达微信的三条路：**Server酱**（经「方糖」服务号发到微信，免费版每天限 5 条）、**企业微信自建应用**（创建应用后在「我的企业 → 微信插件」用个人微信扫码关注，应用消息即同步到微信「企业微信通知」，免费无限制）、**WxPusher**（关注公众号即收，免费无限制）。另有企业微信群机器人 webhook（仅内部群可用）。A/B/C/E 类提醒和高分信号会合并为一条摘要推送；推送失败只记日志，绝不影响扫描。
...
按三阶段入场框架自动建立虚拟仓位：阶段一 25%、阶段二累计 60%、阶段三 100%（`paper_fills`）。每次扫描用最新收盘估值（`paper_valuations`），日报输出组合净值指数（1.0000 起步）。仅供研究对照，不涉及真实下单。
...
系统分别统计 Score ≥ 6/7/8/9/10/11 后 3、5、10、20、60 个交易日的胜率、平均收益、中位收益、最大回撤、盈亏比和样本量，并单列普通交易日、月末/月初、季度末/季度初。季度窗口是否有效由数据结果判断，不预设结论。

回测同时内置**压力位过滤实验**：对比"临近压力位（≤3%）"与"非临近"两组信号的 5 日胜率/收益，用于决定是否要在实盘信号中降级被压制的候选；逐事件 CSV 含 `near_resistance`、`breakout` 列。

防未来函数措施：所有指标在信号日逐行计算；新低和均量基准先排除当日；每个回测日期传给评分函数的行情在该日截断；支撑/压力位只用目标日之前的摆动点；基本面执行 `as_of` 过滤；未来收益仅在评分冻结后由独立函数计算。
...
扫描器内置**数据看门狗**：若某市场基准数据缺失或过期（数据源延迟发布），自动对该市场标的重试一次取数，仍失败才标记 partial 并继续其他市场——避免单一数据源晚发布导致整市场弃扫。
...
- 回测未计手续费、滑点、涨跌停、汇率、税、分红现金流和成交容量；逐日重叠信号也不是统计独立样本。
