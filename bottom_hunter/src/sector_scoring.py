...
def calculate_sector_score(
    ...
    if not rows:
        ...
        return SectorResult(target, sector_id, sector_name, market, 0, components, breadth, [])
    ...
    leader_values = [
        (signal_map[symbol].score.rejection / 2) + (0.5 if signal_map[symbol].relative_strength_turn else 0)
    ...
    if etf_frame is not None:
        ...
        if stamp in etf.index:
            ...
            etf_points = 10 * (0.5 * float(etf_row["return_1d"] > 0) + 0.5 * float(etf_row["close"] > etf_row["ma5"]))
    volume_confirmations = [float(row["volume_ratio"] >= 1.2 and row["close_position"] >= 0.55) for row in rows]
    volume_points = 10 * float(np.mean(volume_confirmations))
    pattern_points = 10 * float(
        np.mean(
            [
                bool(row["bullish_engulfing"] or row["morning_star"] or row["long_lower_shadow"] or row["higher_low_2"])
                for row in rows
            ]
        )
    )
